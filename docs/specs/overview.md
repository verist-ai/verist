...
After calling `runStep()`, the caller **must** complete the following to honor the kernel's guarantees. Below, `stepResult` refers to the unwrapped `StepResult` (i.e., `result.value` when `result.ok === true`).
...
```typescript
const result = await runStep({ step, input, contextFactory, ... });
if (result.ok) {
  // 1. Persist delta
  await store.commit({
